...
import jp.ac.it.shibaura.android.ambient.RTMonAndroidOfAmbientTemperature;
...
public class RTMonAndroidImpl extends RTCBase {
	...
	private static RTMonAndroidOfAmbientTemperature	myRTC = null;
	...
	// OutPort
	private OutPort<TimedDouble>		outPortDoubleTa;
	
	TimedDouble dataTa = new TimedDouble();
	 ...
	 * @param mode データ型
	 */
	public RTMonAndroidImpl(RTMonAndroidOfAmbientTemperature myRTC, RTCService rtcService) {
	...
	private void initOutPort(){
		String TaName = RTMonAndroidProfile.OutPort1;
		TimedDouble Ta = new TimedDouble();
		outPortDoubleTa = new OutPort<TimedDouble>(TaName, Ta);
		rtcService.addOutPort(outPortDoubleTa);
	}
	...
	private void ioControl(){
		...
		outPortDoubleTa.write(dataTa);						// データをoutPortに書き出す。
		Logger4RTC.debug(TAG, "onExecute");
