 ...
 */
package jp.ac.it.shibaura.android.ambient;
 ...
 * RTMonAndroid
 */
public class RTMonAndroidOfAmbientTemperature extends Activity {
	...
	private double dataTa = 0;			//周辺温度センサ情報
	...
	public void onResume() {
		...
		List<Sensor> ss = sm.getSensorList(Sensor.TYPE_AMBIENT_TEMPERATURE);
		if(ss.size() > 0){
			s = ss.get(0);
	...
	private class NameServerConnectListenerImpl implements NameServerConnectListener {
		...
		public void onConnected() {
			Intent intent = new Intent(RTMonAndroidOfAmbientTemperature.this, RTCService.class);
			startService(intent);
	...
	private class RtcServiceConnection implements ServiceConnection {
		...
		public void onServiceConnected(ComponentName className, IBinder service) {
			...
			rtcImpl = new RTMonAndroidImpl(RTMonAndroidOfAmbientTemperature.this, rtcService);
	...
	/**
	 * センサの情報を監視して、周辺温度センサのデータが変化するたびに、
	 * プログラム内の周辺温度センサ用変数の値を更新する
	 * TYPE_AMBIENT_TEMPERATURE == 周辺温度センサ
	 */
