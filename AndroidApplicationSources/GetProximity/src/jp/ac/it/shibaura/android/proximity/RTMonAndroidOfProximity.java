 ...
 */
package jp.ac.it.shibaura.android.proximity;
 ...
 * RTMonAndroid
 */
public class RTMonAndroidOfProximity extends Activity {
	...
	private double dataCm = 0;			//近接センサの距離情報
	...
	public void onResume() {
		...
		List<Sensor> ss = sm.getSensorList(Sensor.TYPE_PROXIMITY);
		if(ss.size() > 0){
			s = ss.get(0);
	...
	private class NameServerConnectListenerImpl implements NameServerConnectListener {
		...
		public void onConnected() {
			Intent intent = new Intent(RTMonAndroidOfProximity.this, RTCService.class);
			startService(intent);
...
//			rtcService.setLongConfig(RTMonAndroidProfile.ConfigName1, 0);
			rtcImpl = new RTMonAndroidImpl(RTMonAndroidOfProximity.this, rtcService);
	...
	/**
	 * センサの情報を監視して、近接センサのデータが変化するたびに、
	 * プログラム内の近接センサ用変数の値を更新する
