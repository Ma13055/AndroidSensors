 ...
 */
package jp.ac.it.shibaura.android.gyroscope;
 ...
 * RTMonAndroid
 */
public class RTMonAndroidOfGyroscope extends Activity {
	...
	public void onResume() {
		...
		List<Sensor> ss = sm.getSensorList(Sensor.TYPE_GYROSCOPE);
		if(ss.size() > 0){
			s = ss.get(0);
	...
	private class NameServerConnectListenerImpl implements NameServerConnectListener {
		...
		public void onConnected() {
			Intent intent = new Intent(RTMonAndroidOfGyroscope.this, RTCService.class);
			startService(intent);
	...
	private class RtcServiceConnection implements ServiceConnection {
		...
		public void onServiceConnected(ComponentName className, IBinder service) {
			...
			rtcImpl = new RTMonAndroidImpl(RTMonAndroidOfGyroscope.this, rtcService);
	...
	/**
	 * センサの情報を監視して、ジャイロセンサのデータが変化するたびに、
	 * プログラム内のジャイロセンサ用変数の値を更新する
