 ...
 */
package jp.ac.it.shibaura.android.gravity;
 ...
 * RTMonAndroid
 */
public class RTMonAndroidOfGravity extends Activity {
	...
	private double dataX = 0;			//重力センサx軸情報
	private double dataY = 0;			//重力センサy軸情報
	private double dataZ = 0;			//重力センサz軸情報
	...
	public void onResume() {
		...
		List<Sensor> ss = sm.getSensorList(Sensor.TYPE_GRAVITY);
		if(ss.size() > 0){
			s = ss.get(0);
	...
	private class NameServerConnectListenerImpl implements NameServerConnectListener {
		...
		public void onConnected() {
			Intent intent = new Intent(RTMonAndroidOfGravity.this, RTCService.class);
			startService(intent);
	...
	private class RtcServiceConnection implements ServiceConnection {
		...
		public void onServiceConnected(ComponentName className, IBinder service) {
			...
			rtcImpl = new RTMonAndroidImpl(RTMonAndroidOfGravity.this, rtcService);
	...
	/**
	 * センサの情報を監視して、重力センサのデータが変化するたびに、
	 * プログラム内の重力センサ用変数の値を更新する
	 * TYPE_GRAVITY == 重力センサ
	 */
