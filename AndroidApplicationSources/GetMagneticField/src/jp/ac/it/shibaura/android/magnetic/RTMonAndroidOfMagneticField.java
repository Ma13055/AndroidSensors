 ...
 */
package jp.ac.it.shibaura.android.magnetic;
 ...
 * RTMonAndroid
 */
public class RTMonAndroidOfMagneticField extends Activity {
	...
	private double dataX = 0;			//磁気センサx軸情報
	private double dataY = 0;			//磁気センサy軸情報
	private double dataZ = 0;			//磁気センサz軸情報
	...
	public void onResume() {
		...
		List<Sensor> ss = sm.getSensorList(Sensor.TYPE_MAGNETIC_FIELD);
		if(ss.size() > 0){
			s = ss.get(0);
	...
	private class NameServerConnectListenerImpl implements NameServerConnectListener {
		...
		public void onConnected() {
			Intent intent = new Intent(RTMonAndroidOfMagneticField.this, RTCService.class);
			startService(intent);
	...
	private class RtcServiceConnection implements ServiceConnection {
		...
		public void onServiceConnected(ComponentName className, IBinder service) {
			...
			rtcImpl = new RTMonAndroidImpl(RTMonAndroidOfMagneticField.this, rtcService);
	...
	/**
	 * センサの情報を監視して、磁気センサのデータが変化するたびに、
	 * プログラム内の磁気センサ用変数の値を更新する
	 * TYPE_MAGNETIC_FIELD == 磁気センサ
	 */
