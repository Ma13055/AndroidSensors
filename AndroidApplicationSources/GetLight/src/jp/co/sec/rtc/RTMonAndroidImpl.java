...
import jp.ac.it.shibaura.android.light.RTMonAndroidOfLight;
...
public class RTMonAndroidImpl extends RTCBase {
	...
	private static RTMonAndroidOfLight	myRTC = null;
	...
	// OutPort
	private OutPort<TimedDouble>		outPortDoubleLux;
	
	TimedDouble dataLux = new TimedDouble();
	 ...
	 * @param mode データ型
	 */
	public RTMonAndroidImpl(RTMonAndroidOfLight myRTC, RTCService rtcService) {
	...
	private void initOutPort(){
		String LName = RTMonAndroidProfile.OutPort1;
		TimedDouble Lux = new TimedDouble();
		outPortDoubleLux = new OutPort<TimedDouble>(LName, Lux);
		rtcService.addOutPort(outPortDoubleLux);
	}
	...
	private void ioControl(){
		...
		outPortDoubleLux.write(dataLux);						// データをoutPortに書き出す。
		Logger4RTC.debug(TAG, "onExecute");
