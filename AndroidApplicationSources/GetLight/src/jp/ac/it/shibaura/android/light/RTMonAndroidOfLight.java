 ...
 */
package jp.ac.it.shibaura.android.light;
 ...
 * RTMonAndroid
 */
public class RTMonAndroidOfLight extends Activity {
	...
	private double dataLux = 0;			//照度センサの距離情報
	...
	public void onResume() {
		...
		List<Sensor> ss = sm.getSensorList(Sensor.TYPE_LIGHT);
		if(ss.size() > 0){
			s = ss.get(0);
	...
	private class NameServerConnectListenerImpl implements NameServerConnectListener {
		...
		public void onConnected() {
			Intent intent = new Intent(RTMonAndroidOfLight.this, RTCService.class);
			startService(intent);
	...
	private class RtcServiceConnection implements ServiceConnection {
		...
		public void onServiceConnected(ComponentName className, IBinder service) {
			...
			rtcImpl = new RTMonAndroidImpl(RTMonAndroidOfLight.this, rtcService);
	...
	/**
	 * センサの情報を監視して、照度センサのデータが変化するたびに、
	 * プログラム内の照度センサ用変数の値を更新する
	 * TYPE_PROXIMITY == 照度センサ
	 */
