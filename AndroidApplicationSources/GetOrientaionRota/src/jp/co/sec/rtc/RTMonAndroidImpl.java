...
import jp.ac.it.shibaura.android.orientaionRota.RTMonAndroidOfOrientation;
...
public class RTMonAndroidImpl extends RTCBase {
	...
	public int onExecute() {
		...
		if(myRTC.getSensorBoolean() == true){
			...
			String str = "";
			str = "傾きセンサ[rad:deg]\n"
					+ "方位角 : " + "( " + String.format("%3.1f",Math.toDegrees(dataAzimuth.getData())) + " ) : " + dataAzimuth.getData() + "\n"
					+ "傾斜角 : " + "( " + String.format("%3.1f",Math.toDegrees(dataPitch.getData())) + " ) : " + dataPitch.getData() + "\n"
					+ "回転角 : " + "( " + String.format("%3.1f",Math.toDegrees(dataRoll.getData())) + " ) : " + dataRoll.getData();
