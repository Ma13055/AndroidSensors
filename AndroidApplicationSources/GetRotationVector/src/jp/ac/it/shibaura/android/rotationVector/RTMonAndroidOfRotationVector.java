 ...
 */
package jp.ac.it.shibaura.android.rotationVector;
 ...
 * RTMonAndroid
 */
public class RTMonAndroidOfRotationVector extends Activity {
	...
	private double dataX = 0;			//回転ベクトルセンサ情報 x*sin(θ/2)
	private double dataY = 0;			//回転ベクトルセンサ情報 y*sin(θ/2)
	private double dataZ = 0;			//回転ベクトルセンサ情報 z*sin(θ/2)
	private double dataCos = 0;			//回転ベクトルセンサ情報 cos(θ/2)
	private double dataAccuracy = 0;	//回転ベクトルセンサ情報 estimated heading Accuracy (in radians) (-1 if unavailable)
	...
	public void onResume() {
		...
		List<Sensor> ss = sm.getSensorList(Sensor.TYPE_ROTATION_VECTOR);
		if(ss.size() > 0){
			s = ss.get(0);
	...
	private class NameServerConnectListenerImpl implements NameServerConnectListener {
		...
		public void onConnected() {
			Intent intent = new Intent(RTMonAndroidOfRotationVector.this, RTCService.class);
			startService(intent);
...
//			rtcService.setLongConfig(RTMonAndroidProfile.ConfigName1, 0);
			rtcImpl = new RTMonAndroidImpl(RTMonAndroidOfRotationVector.this, rtcService);
	...
	/**
	 * 回転ベクトルセンサのx軸の回転ベクトル情報を返す
	 * @return x軸の回転ベクトル(double)
	 */
	public double getX(){
		return dataX;
	}
	...
	/**
	 * 回転ベクトルセンサのy軸の回転ベクトル情報を返す
	 * @return y軸の回転ベクトル(double)
	 */
	public double getY(){
		return dataY;
	}	
	...
	/**
	 * 回転ベクトルセンサのz軸の回転ベクトル情報を返す
	 * @return z軸の回転ベクトル(double)
	 */
	public double getZ(){
		return dataZ;
	}
	...
	/**
	 * センサの情報を監視して、回転ベクトルセンサのデータが変化するたびに、
	 * プログラム内の回転ベクトルセンサ用変数の値を更新する
