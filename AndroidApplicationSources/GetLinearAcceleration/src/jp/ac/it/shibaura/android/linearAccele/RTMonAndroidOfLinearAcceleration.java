 ...
 */
package jp.ac.it.shibaura.android.linearAccele;
 ...
 * RTMonAndroid
 */
public class RTMonAndroidOfLinearAcceleration extends Activity {
	...
	private double dataX = 0;			//直線加速度センサx軸情報
	private double dataY = 0;			//直線加速度センサy軸情報
	private double dataZ = 0;			//直線加速度センサz軸情報
	...
	public void onResume() {
		...
		List<Sensor> ss = sm.getSensorList(Sensor.TYPE_LINEAR_ACCELERATION);
		if(ss.size() > 0){
			s = ss.get(0);
	...
	private class NameServerConnectListenerImpl implements NameServerConnectListener {
		...
		public void onConnected() {
			Intent intent = new Intent(RTMonAndroidOfLinearAcceleration.this, RTCService.class);
			startService(intent);
...
//			rtcService.setLongConfig(RTMonAndroidProfile.ConfigName1, 0);
			rtcImpl = new RTMonAndroidImpl(RTMonAndroidOfLinearAcceleration.this, rtcService);
	...
	/**
	 * センサの情報を監視して、直線加速度センサのデータが変化するたびに、
	 * プログラム内の直線加速度センサ用変数の値を更新する
	 * TYPE_LINEAR_ACCELERATION == 直線加速度センサ
	 */
